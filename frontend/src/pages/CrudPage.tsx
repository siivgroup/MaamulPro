...
const CrudPage = ({ title, description, endpoint, fields, canCreate = true, canEdit = true, canDelete = true, createPermission, updatePermission, deletePermission, transitions = [], initialMode, recordId, returnTo, printable = false, initialValues, standalone = false, createTo, editTo }: CrudPageProps) => {
    ...
    const printRecord = (row: Record<string, any>) => {
        ...
        printWindow.document.write(`<html><head><title>${escape(title)}</title><style>*{box-sizing:border-box}body{font:14px Arial;margin:0;padding:32px;color:#172033;overflow-wrap:anywhere}h1{margin:0 0 4px}p{color:#64748b}table{border-collapse:collapse;table-layout:fixed;width:100%;margin-top:24px}th,td{border:1px solid #dbe2ea;padding:10px;text-align:left;vertical-align:top;overflow-wrap:anywhere}th{width:28%;background:#f6f8fb}@page{margin:12mm}@media print{body{padding:0;font-size:10pt}button{display:none}tr{break-inside:avoid}}</style></head><body><h1>${escape(title)}</h1><p>MaamulPro · ${escape(new Date().toLocaleString())}</p><table>${rowsHtml}</table><button onclick="window.print()" style="margin-top:20px;padding:10px 18px">Print</button></body></html>`);
